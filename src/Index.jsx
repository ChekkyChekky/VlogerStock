...
class IndexComp extends Component {
    ...
    hadleGetChannelFromSearchTerm(data) {
        this.handleGetChannelDetailsStrictID(data.id.channelId);
    }

    handleGetChannelFromTitle(searchTerm) {
        YTSearchTerm({ key: API_KEY, term: searchTerm, maxResults: 5 },
            (data) => {
                if (data[0].id.kind === "youtube#channel") {
                    this.hadleGetChannelFromSearchTerm(data[0]);
                }
            });
    }
        ...
        YTChannels({ key: API_KEY, id: channelID },
            (data) => {
                if (data.length !== 0) {
                    ...
                    this.handleGetChannelFromTitle(channelID);
                }
